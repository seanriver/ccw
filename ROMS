#!/bin/csh -f
#
# svn $Id: build.sh 893 2018-02-11 03:54:11Z arango $
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
if ($USE_MY_LIBS == 'yes') then
  ...
  switch ($FORT)
    ...
    case "gfortran"
      ...
      setenv ARPACK_LIBDIR        /usr/lib
      if ($?USE_MPI) then
        if ($which_MPI == "mpich2" ) then
          setenv ESMF_DIR         /opt/mpich2/bin
          setenv MCT_INCDIR       /usr/local/include
          setenv MCT_LIBDIR       /usr/local/lib
          setenv PARPACK_LIBDIR   /usr/local/lib
        else if ($which_MPI == "openmpi" ) then
          setenv ESMF_DIR         /opt/openmpi/bin
          setenv MCT_INCDIR       /usr/local/include
          setenv MCT_LIBDIR       /usr/local/lib
          setenv PARPACK_LIBDIR   /usr/local/lib
        endif
      ...
      if ($?USE_NETCDF4) then
        if ($?USE_PARALLEL_IO && $?USE_MPI) then
          if ($which_MPI == "mpich2" ) then
            ...
            setenv NF_CONFIG      /usr/local/bin/nf-config
